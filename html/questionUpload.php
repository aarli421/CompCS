...
if (isset($_FILES['questionInput']) && isset($_POST['unlock_value']) && isset($_POST['bonus']) && isset($_POST['testcase_value']) && isset($_POST['prompt'])) {
    ...
    if (!hasValue($msg)) {
        ...
        $sth = $db->prepare("INSERT INTO `questions` (`name`, `prompt`, `unlock_value`, `testcase_value`, `testcases`, `bonus`) VALUES (?, ?, ?, ?, ?, ?);");
        $sth->execute([$name[0], $_POST['prompt'], $_POST['unlock_value'], $_POST['testcase_value'], $testAmount, $_POST['bonus']]);
        ?>
        <script>
